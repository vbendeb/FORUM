...
if (Moderation = "Y" and Topic_Status > 1 and Topic_Author <> MemberID) then
	...
	if iReplyCount = "" then  '## No replies found in DB
		...
		for iForum = 0 to iReplyCount
			...
			Response.Write	"                <font color=""" & strForumFontColor & """ face=""" & strDefaultFontFace & """ size=""" & strFooterFontSize & """><small>" & Reply_MemberPosts & " Posts</small></font></p>" + GetPhotoLinkStr(Reply_Author) + "</td>" & vbNewLine & _
...
sub GetFirst()
	...
	Response.Write	"                <font color=""" & strForumFontColor & """ face=""" & strDefaultFontFace & """ size=""" & strFooterFontSize & """><small>" & Member_Posts & " Posts</small></font></p>" + GetPhotoLinkStr(Topic_Author) + "</td>" & vbNewLine & _
	...
	if strEditedByDate = "1" and Topic_LastEditBy <> "" then
		...
		Response.Write	"                    <tr>" & vbNewLine & _
				...
				"Edited by - " & Topic_LastEditByName & " on " & chkDate(Topic_LastEdit, " ", true) & "</font>" & vbNewLine & _
				"                    </tr>" & vbNewLine
	end if
