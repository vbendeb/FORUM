...
if strProhibitNewMembers <> "1" then
	if Request.QueryString("mode") <> "DoIt" and Request.QueryString("actkey") = "" then
		...
		if strAuthType = "nt" and ChkAccountReg = "1" then
			Response.Write	"      " & strParagraphFormat1 & "<b>Registration for this account is not necessary.</b></font></p>" & vbNewLine & _
...
'			end if
			...
			if strEmail = "1" and strRestrictReg = "0" then
				...
				strMessage = strMessage & "Вы получили это сообщение от " & strForumTitle & " потому что вы запросили доступ к сообщениям участников форума" & vbNewline & vbNewline
				if strAuthType="db" then
				'################################### E-mail Validation Mod #################################
					if strEmailVal = "1" then
						...
						strMessage = strMessage & "Если Вы испытываете проблемы с регистрацией - напишите по адресу webmaster@moct.org" & vbNewline & vbNewLine
					else
				'######################################################################################
...
<%
			...
			Response.Write	"      <p align=""center""><font face=""" & strDefaultFontFace & """ size=""" & strHeaderFontSize & """ color=""" & strHiLiteFontColor & """>Проблема с вашими данными</font></p>" & vbNewLine & _
					...
					"        </tr>" & vbNewLine & _
					"      </table>" & vbNewLine & _
					"      " & strParagraphFormat1 & "<a href=""JavaScript:history.go(-1)"">" & strGBTED & "</a></font></p>" & vbNewLine
			WriteFooter
			Response.End
		end if
		' ##################### E-mail Validation Mod #########################
		if lcase(strEmail) = "0" then
			...
			'#######################################
				if strRestrictReg = "1" then
					...
					Response.Write	"      " & strParagraphFormat1 & "Пожалуйста выполните действия перечисленные в сообщении посланном по этому <b>" & ChkString(Request.Form("Email"),"адресу") & "</b> адресу чтобы завершить Вашу регистрацию.</font></p>" & vbNewLine
				end if
			'#######################################
		 	else
				Response.Write	"      <p align=""center""><font face=""" & strDefaultFontFace & """ size=""" & strHeaderFontSize & """>Ваша регистрация успешно завершена!</font></p>" & vbNewLine & _
...
sub ShowForm()
	Response.Write	"      <form action=""register.asp?mode=DoIt"" method=""Post"" id=""Form1"" name=""Form1"">" & vbNewLine & _
			"      <input name=""Refer"" type=""hidden"" value=""" & Request.Form("Refer") & """>" & vbNewLine & _
