...
'--------------------------------------------------------------
' Ш.Е. Функции StrHexСoder и StrHexDecoder добавлены в форум,
' для кодирования/декодирования куки логина при их записи/чтении
' на ПК клиента. Это позволяет решить проблему работы с кириллицей
' при использовании куки. Исходная строка заменяется на строку
' шестнадцатеричных кодов символов строки.
' Для безболезненного перехода от старого формата куки к новому
' используется удаление куки при декодировании, если в строке
' встречается не шестнадцатеричный символ
' Измененные файлы (изменения находятся по сигнатурам функций
' StrHexСoder и StrHexDecode, оригинальные строки закомментированы):
'  - inc_func_common.asp (этот файл)
'  - inc_header_short.asp
'  - inc_header.asp
'  - admin_config_system.asp
' Кроме того:
'  - config.asp (добавлена строка Session.CodePage = 1251)
